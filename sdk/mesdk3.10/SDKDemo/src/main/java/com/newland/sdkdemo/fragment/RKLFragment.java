 ...
 * @Date 2022/4/13
 */
public class RKLFragment extends BaseFragment{

    public static final int CERT_PRIVATE_KEY_ID = 249;//KRD_AUTH_ID

    public RKLFragment(Context context) {
        super(context, LayoutMode.GRID);
    }

    @Override
    public String title() {
        return context.getString(R.string.tv_rkl_f);
    }
    ...
    public Object getModule() {
        return RKLFragment.this;
    }

    @MethodGridEntity(btnnameid = R.string.rkl_load_testcertkey, functionid = 1)
    private void installTestCertKey(){
        ST_SEC_KEYIN_DATA keyData = new ST_SEC_KEYIN_DATA();
        keyData.ucKeyIdx = CERT_PRIVATE_KEY_ID;
        keyData.KeyUsage = EM_SEC_KEY_USAGE.KEY_USE_ASYM_AUTH.getCode();
        keyData.KeyType = EM_SEC_CRYPTO_KEY_TYPE.KEY_TYPE_ASYM_RSA.getCode();
        keyData.pKeyData = CERT_PRIVATE_KEY;
        keyData.nKeyDataLen = CERT_PRIVATE_KEY.length;
        keyData.nKeyLen = CERT_PRIVATE_KEY.length;
        keyData.pAD = ASYM_CERT;
        keyData.nADSize = ASYM_CERT.length;
        int ret = NdkApiManager.getNdkApiManager().getSecNapi().NAPI_SecGenerateKey(EM_SEC_KEYIN_METHOD.SEC_KIM_CLEAR.ordinal(),
                keyData,new ST_SEC_KCV_DATA());
        if(ret == 0){
            showMessage("installTestCertKey success");
        }else {
            showMessage("installTestCertKey failed");
        }
    }

    @MethodGridEntity(btnnameid = R.string.tv_rkl_f, functionid = 2)
    private void startRKL(){
        RKLParams params = new RKLParams();
        params.setConfigFile("Newland_RKLCONF_DEV");
        params.setRklApi(RKLParams.RKLAPIType.NewlandAPI);
        ModuleManage.getInstance().getPinpadModule().startRKL(params,rklListener);
    }

    private RKLListener rklListener = new RKLListener() {
        @Override
        public void onStatusChanged(RKLStatus rklStatus, int errorCode, String message) {
            if (rklStatus == RKLStatus.ENDED) {
                if (errorCode != RKLErrorCode.SUCCESS) {
                    // FlyKey failed
                    showMessage(String.format("FlyKey failed, status: %s, error code: %d, message: %s", rklStatus, errorCode, message));
                } else {
                    // FlyKey success
                    showMessage(String.format("FlyKey success, status: %s", rklStatus));
                    int installedNum = 0;
                    StringBuilder sb = new StringBuilder();
                    try {
                        installedNum = RKLProcessor.getInstance().getInstalledKeyNum();
                        showMessage(String.format("FlyKey installed key num: %d\n", installedNum));
                    } catch (RKLException e) {
                        e.printStackTrace();
                        showMessage(String.format("%s(%d)", e.getMessage(), e.getCode()));
                    }
                    List<KeyInfo> keys = null;
                    try {
                        keys = RKLProcessor.getInstance().getInstalledKeyInfo();
                ...
                // Update FlyKey status
                showMessage(String.format("FlyKey status: %s", rklStatus));
            }
        }
    };

    private void checkKcv(StringBuilder sb, List<KeyInfo> keys) {
        for (KeyInfo ki : keys) {
            if (!TextUtils.isEmpty(ki.getKcv())) {
                try {
                    int[] len = new int[1];
                    byte[] data = new byte[16];
                    int ret = NdkApiManager.getNdkApiManager().getSecNapi().NAPI_SecGetKeyInfo(1,ki.getIndex(),ki.getType(),
                            ki.getUsage(),null,0,data,len);
                    if(ret == 0){
                        byte[] kcv = new byte[len[0]];
                        System.arraycopy(data,0,kcv,0,kcv.length);
                        String kcvStr = ISOUtils.hexString(kcv);
                        if (ki.getKcv().startsWith(kcvStr)||kcvStr.startsWith(ki.getKcv())) {
                            sb.append(String.format("Installed key: %s\n", ki.toString()));
                        } else {
                            if (ki.getUsage() == EM_SEC_KEY_USAGE.KEY_USE_DUKPT.getCode()) {//Dukpt
                                sb.append(String.format("Installed key: %s\n", ki.toString()));
                            } else {
                                sb.append(String.format("Installed key but kcv not matched: (response kcv %s)(get kcv %s)\n", ki.toString(), kcvStr));
                            }
                        }
                    }else {
                        sb.append(String.format("Installed key but failed to get kcv: %s\n", ki.toString()));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
