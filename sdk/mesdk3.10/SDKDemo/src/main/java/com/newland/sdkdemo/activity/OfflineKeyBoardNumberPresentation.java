 ...
 */
public class OfflineKeyBoardNumberPresentation extends Presentation {
    private static final DeviceLogger logger = DeviceLoggerFactory.getLogger(OfflineKeyBoardNumberPresentation.class);
    ...
    public OfflineKeyBoardNumberPresentation(Context outerContext, Display display) {
            ...
            public boolean onPreDraw() {
                if (!first) {
                    ...
                    int timeOut = 59;
                        ...
                        byte[] modulus = AppConfig.modulus;//getIntent().getByteArrayExtra("modulus");// The modulus of actual transaction.it is a parameter of EmvTransInfo through emvTransInfo.getModulus().EmvTransInfo cames from onRequestPinEntry of emv callback.
                        byte[] exponent = AppConfig.exponent;//getIntent().getByteArrayExtra("exponent");// The exponent of actual transaction.it is a parameter of EmvTransInfo through emvTransInfo.getExponent().EmvTransInfo cames from onRequestPinEntry of emv callback.
    ...
    private PinInputListener pinInputListener = new PinInputListener() {
        ...
        public void onFinish(int pinblockLen, byte[] pinblock,byte[] ksn) {
            ...
            if (pinblockLen == 0) {
                ...
                Log.i(TAG, context.getString(R.string.keyboard_activity_log_input_success) + ISOUtils.hexString(pinblock)+";ksn:"+(ksn==null?null:ISOUtils.hexString(ksn)));
                Intent i = new Intent();
