...
int g_aCardAtq = 1;
int g_napiIntf = 1;
int g_ndkemvVerFun = 1;
int g_emvspVerFun = 1;
char ndkemvVersion[60];
char emvspVersion[60];
extern int dukptAESFlag;
//kb
...
int (*NDK_LedSetFlickParam)(EM_LED emStatus, ST_NAPI_LED_FLICK FlickParam);
...
/**
 * EMV���װ��RfidPiccDetect,libNDK_EMV.so
 * @param psPiccType
 * @return
 */
int (*NDK_NL_RfidPiccDetect)(uchar *psPiccType);
char* (*NDK_EMVGetVersion)(void);
char* (*EMV_SpGetVersion)(void);

static int g_hasSecModule = 1;
static int NDK_Null()
{
//    __android_log_print(ANDROID_LOG_DEBUG, TAG, "into NDK_Null %x", NDK_Null);
	return SDK_ERR_NDK_NOT_SUPPORT;
}
...
{
	__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>Nonsupport[%s]",foo);
	if(!strcmp(foo,"NDK_M1Anti_SEL")||!strcmp(foo,"NDK_M1Select_SEL"))
	{
		g_rfMultiLevel = 0;
		__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>Nonsupport[%s] g_rfMultiLevel[%d]",foo,g_rfMultiLevel);
	}
	if(!strcmp(foo,"NDK_RfidPiccDetect_Atq"))
	{
		g_aCardAtq = 0;
		__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>Nonsupport[%s] g_aCardAtq[%d]",foo,g_aCardAtq);
	}
	if(!strcmp(foo,"NAPI_SecGenerateKey"))
	{
		g_napiIntf = 0;
		__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>Nonsupport[%s] g_napiIntf[%d]",foo,g_napiIntf);
	}

	if(!strcmp(foo,"NDK_EMVGetVersion"))
	{
		g_ndkemvVerFun = 0;
		__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>Nonsupport[%s] g_ndkemvVerFun[%d]",foo,g_ndkemvVerFun);
	}
	if(!strcmp(foo,"EMV_SpGetVersion"))
	{
		g_emvspVerFun = 0;
		__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>Nonsupport[%s] g_emvspVerFun[%d]",foo,g_emvspVerFun);
	}
}
static void Check_HasSecModule()
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>persist.sys.HasSecModule[%s] g_hasSecModule[%d] ret[%d]",propBuf,g_hasSecModule,ret); 
}
...
int Ndk_Dlload(){
	...
	dlError = (char *)dlerror();
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libnlposapi.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);

	if(functionLib == NULL){
		functionLib = dlopen("libnlposapi.npt.so",RTLD_LAZY);
		dlError = (char *)dlerror();
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libnlposapi.npt.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
	}
	...
	dlError = (char *)dlerror();
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libnl_ndk.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);

	if(functionLib == NULL){
		functionLib = dlopen("libnl_ndk.npt.so",RTLD_LAZY);
		dlError = (char *)dlerror();
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libnl_ndk.npt.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
	}
    ...
    __android_log_print(ANDROID_LOG_DEBUG, TAG, ">>>eventFlag[%d]", eventFlag);
    if(!eventFlag){
	...
	dlError = (char *)dlerror();
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libnlprnapi.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
	...
	dlError = (char *)dlerror();
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libnlprintex.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
...
//	functionLib = dlopen("libNDK_EMV.so",RTLD_LAZY);
//	dlError = (char *)dlerror();
//	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen libNDK_EMV.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
//	functionLib = NULL;
//	DLSYM(functionLib, NDK_NL_RfidPiccDetect);

	memset(ndkemvVersion,0,sizeof(ndkemvVersion));
	memset(emvspVersion,0,sizeof(emvspVersion));
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "sizeof(ndkemvVersion)[%d]",sizeof(ndkemvVersion));

	functionLib = dlopen("libNDK_EMV.so",RTLD_LAZY);
	dlError = (char *)dlerror();
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen2. libNDK_EMV.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
	DLSYM(functionLib, NDK_EMVGetVersion);
	if(g_ndkemvVerFun == 1){
		char* emvVersion2 = NDK_EMVGetVersion();
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "ndkemv emvVersion2[%s] strlen(emvVersion2)[%d]\n", emvVersion2,strlen(emvVersion2));

		functionLib = dlopen("/data/share/libNDK_EMV.so",RTLD_LAZY);
		dlError = (char *)dlerror();
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen. libNDK_EMV.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
		DLSYM(functionLib, NDK_EMVGetVersion);
		if(g_ndkemvVerFun == 1){
			char* emvVersion = NDK_EMVGetVersion();
			__android_log_print(ANDROID_LOG_DEBUG, TAG, "ndkemv emvVersion[%s]\n", emvVersion);
			int ret = strcmp(emvVersion2,emvVersion);
			if(ret < 0){//s1<s2
				memcpy(ndkemvVersion,emvVersion,strlen(emvVersion));
			}else{//s1>s2,s1=s2
				memcpy(ndkemvVersion,emvVersion2,strlen(emvVersion2));
			}
		} else{
			memcpy(ndkemvVersion,emvVersion2,strlen(emvVersion2));
		}
	...
	functionLib = dlopen("libemv_sp.so",RTLD_LAZY);
	dlError = (char *)dlerror();
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen2. libemv_sp.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
	DLSYM(functionLib, EMV_SpGetVersion);
	if(g_emvspVerFun == 1){
		char* spVersion2 = EMV_SpGetVersion();
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "ndkemv spVersion2[%s]\n", spVersion2);

		functionLib = dlopen("/data/share/libemv_sp.so",RTLD_LAZY);
		dlError = (char *)dlerror();
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "dlopen. libemv_sp.so dlError[%s] functionLib[0x%x]\n", dlError,functionLib);
		DLSYM(functionLib, EMV_SpGetVersion);
		if(g_emvspVerFun == 1){
			char* spVersion = EMV_SpGetVersion();
			__android_log_print(ANDROID_LOG_DEBUG, TAG, "ndkemv spVersion[%s]\n", spVersion);
			int ret = strcmp(spVersion2,spVersion);
			if(ret < 0){//s1<s2
				memcpy(emvspVersion,spVersion,strlen(spVersion));
			}else{//s1>s2,s1=s2
				memcpy(emvspVersion,spVersion2,strlen(spVersion2));
			}
		} else{
			memcpy(emvspVersion,spVersion2,strlen(spVersion2));
		}
	...
	__android_log_print(ANDROID_LOG_DEBUG, TAG, "ndk_dlload = %d",rc);
	return rc;
...
void NDK_UnRegisterEvent(int events){
	...
	for(;idx <sizeof(ndkevents) / sizeof(int); idx++){
		...
		EXEC_NDK("#NDK_SYS_UnRegisterEvent",NDK_SYS_UnRegisterEvent(ndkevents[idx]),NDK_OK,COMMAND_NONE);
		if(events&SYS_EVENT_PIN){
			uchar sPinBlock[32],szPinKsn[16];
			int nStatus = 0x8000 | K_ESC;
            //if(getUseNapi())
			LOGD_FMT("NAPI_SEC_VPP_KEY_ESC/");
			EXEC_NDK("NAPI_SecVPPSetEvent", NAPI_SecVPPSetEvent(NAPI_SEC_VPP_KEY_ESC),NDK_OK,PINPAD_INPUT);
			uint event;
			uchar pinBlock[32];int pinBlockLen = 0;
			uchar ksn[32];int ksnLen = 0;
			memset(pinBlock,0, sizeof(pinBlock));
			memset(ksn,0, sizeof(ksn));
			EXEC_NDK("NAPI_SecVPPGetEvent", NAPI_SecVPPGetEvent(&event,pinBlock,&pinBlockLen,ksn,&ksnLen),NDK_OK,PINPAD_INPUT);
			//else
...
static int getEnableNapi(){
	return (dukptAESFlag || 0);
}

int getUseNapi(){
	int isUse = getEnableNapi()&&getSupportNapi();
	LOGD_FMT("isUse[%d]",isUse);
	return isUse;
}