...
int NAPI_L3GetTlvData(uchar *tagList,int tagListLen, unsigned int tagNum, uchar KeyIndex, uchar *tlvData, unsigned int maxLen,int ctl,int *realLen)
{
    ...
    NDK_IntToC2(SendData + 5, tagListLen);
...
//	PubUpdateWindow();
    ...
    if(memcmp(RecvData+index, "00", 2) == 0) {
        ...
        NDK_C4ToInt((unsigned int*)&Errorcode, RecvData + index);index+=4;
        ...
        if(flag1F8131){
            ...
            if(status == 0){//succ
                ...
                NDK_C2ToInt((unsigned int*)&tempLen, RecvData+index);index+=2;
                if(keyIndex == 0){
                    tlvLen = tempLen;
                    ...
                    NDK_C2ToInt((unsigned int*)&tlvLen, RecvData+index);index+=2;
                    (*env)->SetIntField(env,txnResultObj,(*env)->GetFieldID(env,txnResultCls, "tlvLen", "I"),tlvLen);
                }
        ...
        if(recvDataLen > index){
            ...
            int l3tlvLen = 0;
            NDK_C4ToInt((unsigned int*)&l3tlvLen, RecvData+index);index+=4;
...
//	PubUpdateWindow();
    ...
    if(memcmp(RecvData, "00", 2) == 0) {
        NDK_C4ToInt((unsigned int*)&Errorcode, RecvData+2);
