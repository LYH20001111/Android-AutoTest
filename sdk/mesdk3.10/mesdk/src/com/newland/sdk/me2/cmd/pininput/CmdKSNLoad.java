...
@CommandEntity(cmdCode = {(byte) 0x1A, (byte) 0x17}, responseClass = com.newland.sdk.me2.cmd.pininput.CmdKSNLoad.CmdKSNLoadResponse.class)
public class CmdKSNLoad extends CommonDeviceCommand {

    @InstructionField(name = "Key模式", index = 0, fixLen = 1, maxLen = 1, serializer = IntegerSerializer.class)
    private int keytype;
    ...
    @InstructionField(name = "装载的初始密钥数据", index = 3, maxLen = 32, serializer = ByteArrSerializer.class)
    ...
    public CmdKSNLoad(int keytype, int KSNIndex, byte[] ksn, byte[] defaultKeyData, int mainKeyIndex, byte[] checkValue) {
    ...
    public static class CmdKSNLoadResponse extends AbstractSuccessResponse {
        ...
        @InstructionField(name = "校验值", index = 1, fixLen = 8, maxLen = 8, serializer = ByteArrSerializer.class)
        ...
        public byte[] getResponseCheckValue() {
            return responseCheckValue;
        }
    }
