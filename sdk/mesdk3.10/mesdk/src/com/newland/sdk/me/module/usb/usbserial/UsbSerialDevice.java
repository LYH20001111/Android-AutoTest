...
import com.newland.sdk.me.module.usb.usbserial.deviceids.CH34xIds;
import com.newland.sdk.me.module.usb.usbserial.deviceids.CP210xIds;
import com.newland.sdk.me.module.usb.usbserial.deviceids.FTDISioIds;
import com.newland.sdk.me.module.usb.usbserial.deviceids.PL2303Ids;
...
public abstract class UsbSerialDevice implements UsbSerialInterface
{
    ...
    {
        Utils.d("UsbSerialDevice syncWrite asyncMode="+asyncMode);
        if(!asyncMode)
        {
            if(buffer == null)
                return 0;
            Utils.d("UsbSerialDevice syncWrite bulkTransfer start timeout="+timeout);
            int ret = connection.bulkTransfer(outEndpoint, buffer, buffer.length, timeout);
            Utils.d("UsbSerialDevice syncWrite bulkTransfer end ret="+ret);
            return ret;
        }else
        {
            Utils.d("UsbSerialDevice syncWrite return -1.");
            return -1;
        }
    }
...
//        Utils.d("UsbSerialDevice syncRead asyncMode="+asyncMode);
        if(asyncMode)
        {
            return -1;
        }
...
//        Utils.d("UsbSerialDevice syncRead bulkTransfer start timeout="+timeout);
        int ret = connection.bulkTransfer(inEndpoint, buffer, offset, length, timeout);
//        Utils.d("UsbSerialDevice syncRead bulkTransfer end ret="+ret);
        return ret;
    }
    ...
    public static boolean isCdcDevice(UsbDevice device)
    {
        ...
        {
            Log.d(TAG,"i"+i+" vid="+vid+" pid="+pid+" cdc.");

            if((i==4 && vid == CDCSerialDevice.CDC_VID_1E0E && pid == CDCSerialDevice.CDC_PID_902E)||
                    (i==2 && vid == CDCSerialDevice.CDC_VID_1E0E && pid == CDCSerialDevice.CDC_PID_9031)){
                Log.d(TAG,"vid="+vid+" pid="+pid+" cdc.");
                return true;
            }

            if(i==2 && vid == CDCSerialDevice.CDC_VID_05C6 &&
                    (pid == CDCSerialDevice.CDC_PID_9102 || pid == CDCSerialDevice.CDC_PID_9120)){
                Log.d(TAG,"vid="+vid+" pid="+pid+" cdc.");
                return true;
            }

            if(i==0 && vid == CDCSerialDevice.CDC_VID_1659 && pid == CDCSerialDevice.CDC_PID_9171){
                Log.d(TAG,"vid="+vid+" pid="+pid+" cdc.");
                return true;
            }
    ...
    protected class WorkerThread extends AbstractWorkerThread
    {
        ...
        public void doRun()
        {
            Utils.d("UsbSerialDevice WorkerThread requestWait start");
            UsbRequest request = connection.requestWait();
            Utils.d("UsbSerialDevice WorkerThread requestWait end");
                    ...
                    && request.getEndpoint().getDirection() == UsbConstants.USB_DIR_IN)
            {
                ...
                Utils.d("UsbSerialDevice WorkerThread getDataReceived="+ (data==null?"null": ISOUtils.hexString(data)));
                ...
                // Queue a new request
                Utils.d("UsbSerialDevice WorkerThread requestIN.queue start");
                requestIN.queue(serialBuffer.getReadBuffer(), SerialBuffer.DEFAULT_READ_BUFFER_SIZE);
                Utils.d("UsbSerialDevice WorkerThread requestIN.queue end");
            }
        }
    ...
    private class WriteThread extends AbstractWorkerThread
    {
        ...
        public void doRun()
        {
            ...
            if(data.length > 0){
                Utils.d("UsbSerialDevice WriteThread bulkTransfer data.length="+data.length+" data="+(data==null?"null":ISOUtils.hexString(data)));
                int ret = connection.bulkTransfer(outEndpoint, data, data.length, USB_TIMEOUT);
                Utils.d("UsbSerialDevice WriteThread bulkTransfer ret="+ret);
            }
    ...
    protected class ReadThread extends AbstractWorkerThread
    {
        ...
        public void doRun()
        {
            ...
            int numberBytes;
            if(inEndpoint != null){
                Utils.d("UsbSerialDevice ReadThread bulkTransfer start serialBuffer="+serialBuffer+" connection="+connection);
                numberBytes = connection.bulkTransfer(inEndpoint, serialBuffer.getBufferCompatible(),
                        SerialBuffer.DEFAULT_READ_BUFFER_SIZE, 0);
                Utils.d("UsbSerialDevice ReadThread bulkTransfer end numberBytes="+numberBytes);
            } else{
                numberBytes = 0;
                Utils.d("UsbSerialDevice ReadThread bulkTransfer end numberBytes==0");
            }
    ...
    {
        Utils.d("UsbSerialDevice restartWorkingThread mr1Version="+mr1Version+" workerThread="+workerThread+" readThread="+readThread);
        if(mr1Version && workerThread == null)
        {
            Utils.d("UsbSerialDevice restartWorkingThread start WorkerThread(Read)");
            workerThread = new WorkerThread(this);
            workerThread.start();
            while(!workerThread.isAlive()){} // Busy waiting
        }else if(!mr1Version && readThread == null)
        {
            Utils.d("UsbSerialDevice restartWorkingThread start ReadThread");
    ...
    {
        Utils.d("UsbSerialDevice restartWriteThread writeThread");
        if(writeThread == null)
        {
            Utils.d("UsbSerialDevice restartWriteThread start writeThread");
