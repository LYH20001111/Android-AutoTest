...
public class InnerUtils {
    ...
    */
    ...
    public static String zeropad(long l, int len) {
        ...
        } catch (DeviceRTException ignored) { }
    ...
    /**
     * Turn the byte array into INT
     * 
     * @param bytes Byte array
     * @param offset Offset
     * @param len Length
     * @param isBigEndian Is it big endian
     * 
     * @modify by lance 20140829 不再控制是否为负数
     * @return
     */
...
//    	}
    ...
    /**
     * Automatically determine length to turn into integer
     * 
     * @param value
     * @param isBigEndian
     * 
     * @modify by lance 20140829 No more control over whether it's negative
     * @return
     */
...
//    	}
    ...
    /**
     * Turn the int into the specific byte array
     * 
     * @param value The value to be turned
     * @param len The length of value, the value will be filled with 00 when its length is insufficient
     * @param isBigEndian Is it big endian
     * @return
     * 
     *  @modify by lance 20140829 No more control over whether it's negative
...
//    	}
    ...
    public static long bcdToLong(byte[] input,int offset,int len,boolean padLeft){
    	String intStr = bcd2str(input, offset, len, padLeft);
    ...
    /**
     * Change simple tlv tag to follow ISO/IEC 7816 defined
    ...
    public static int toBERTLVTag(int tag){
    	...
    	if(rslt > 0xFFFFFF){ //max only to 3 bytes，三个字节以上不支持
    		throw new IllegalArgumentException("only support 3 bytes tag" + Dump.getHexDump(intToBytes(rslt, 4, true)));
    	}
    	if(rslt > 0xFFFF){ //长度为3个字节，第一个字节必须是xxx11111,第二个字节必须高位为1，才可能存在第三个字节,第三字节高位必须为0
    		if((rslt & 0x1F8000) != 0x1F8000 || ((rslt & 0x80) == 0x80 )){
    			throw new IllegalArgumentException("illegal tag format:"+Dump.getHexDump(intToBytes(rslt, 3, true)));
    		}
    		//满足条件返回结果
    		return rslt;
    	}
    	if(rslt > 0xFF){ //长度为2个字节,第一个字节必须是xxx11111,第二个字节高位必须为0。
    		if((rslt & 0x1F00) != 0x1F00 || ((rslt & 0x80) == 0x80)){
    			throw new IllegalArgumentException("illegal tag format:"+Dump.getHexDump(intToBytes(rslt, 2, true)));
    		}
    		//满足条件返回结果
    		return rslt;
    	}
    	if((rslt & 0x1F) == 0x1F){ //长度仅为1个字节
    		throw new IllegalArgumentException("illegal tag format:"+Dump.getHexDump(intToBytes(rslt, 1, true)));
    	}
    	return rslt;
    }
    /**
   ...
   public static boolean isBitSet(byte val, int bitPos) {
    ...
    public static byte[] mergeThreeByteArrays(byte[] arr1, byte[] arr2, byte[] arr3) {
