...
*/
extern int
NDK_ScrRectangle(uint unX, uint unY, uint unWidth, uint unHeight, EM_RECT_PATTERNS emFillPattern,
                 color_t unColor);
...
*/
extern int
NDK_KbGetInput(char *pszBuf, uint unMin, uint unMaxLen, uint *punLen, EM_INPUTDISP emMode,
               uint unWaitTime, EM_INPUT_CONTRL emControl);
...
extern int
NDK_PrnSetBDF(uint unXpos, uint unLineSpace, uint unWordSpace, uint unXmode, uint unYmode);
 ...
 *@li	\ref NDK_ERR_NOT_SUPPORT "NDK_ERR_NOT_SUPPORT"        不支持该功能
*/
extern int NDK_PrnFeedPaper(EM_PRN_FEEDPAPER emType);


extern int NDK_PrnQrcode(char *in_str, uchar prn_p, uint qrcode_size, uchar err_corr_level);//打印二维码
extern int NDK_PrnBarcode(char *in_str, uint in_len, uchar prn_p, uchar barcode_width,
                          uint barcode_high);//打印一维码
extern int NDK_PrnImageZoom(char *ptmp13, uchar prn_p, uint pictureSizeX, uint pictureSizeY,
                            uchar yspace);//打印log
extern int (*NDK_Script_Print)(char *prndata, int indata_len);

extern int (*NDK_PrnModuleInit)(void);

extern int (*NDK_PrnCutterInit)(void);
 ...
 */
//typedef enum {
//    TP_SET_PARAM_FOR_GRAYSCALE_LEVE, /*设置打印浓度等级 1-5 */
//    TP_SET_PARAM_FOR_CHARGE_PRINTER_MODE, /* 系统通知打印是否进入充电打印模式 */
//    TP_SET_PARAM_FOR_DRIVER_LOGIC, /* 打印驱动逻辑选择 */
//    TP_SET_PARAM_FOR_FEEL_WELL_SPEED_MODE, /* 打印速度设置到最大的同时保证打印效果，不考虑马达失步风险 1,0*/
//    TP_SET_PARAM_END = 0xFFFFFFFF,
//} EM_PRN_SET_PARAM_TYPE;

typedef enum {
    TP_SET_PARAM_FOR_GRAYSCALE_LEVE = 0,
    TP_SET_PARAM_FOR_CHARGE_PRINTER_MODE = 1,
    TP_SET_PARAM_FOR_DRIVER_LOGIC = 2,
    TP_SET_FEEL_WELL_SPEED_MODE = 3,
    TP_SET_TEMP_VOLT_ACTIVEDOT_LIMIT_MODE = 4,
    TP_SET_PARAM_FOR_TEST_ADJUST_DOTS_NUM = 5,
    TP_SET_PARAM_FOR_BATTERY_HEALTH_LEVEL = 6,
    TP_SUPPORT_OVERHEAT_ABNORMAL_DETECTION = 7,
    TP_SET_PARAM_FOR_SET_PRINTER_QUALITY_MODE = 8, /*精细化打印*/
    TP_SET_PARAM_END = 0xFFFFFFFF,
} EM_PRN_SET_PARAM_TYPE;
...
*/
extern int
NDK_AppDoEvent(const uchar *pszFileName, int nModuleId, const void *pvInEventMsg, int nInlen,
               void *pvOutEventMsg, int nMaxOutLen, int *pnOutLen);
...
*/
extern int
NDK_AppGetInfo(const uchar *pszAppName, int nPos, ST_APPINFO *pstAppInfo, int nSizeofInfo);
...
*/
extern int
(*NDK_AlgRSAKeyPairGen)(int nProtoKeyBit, int nPubEType, ST_RSA_PUBLIC_KEY *pstPublicKeyOut,
                        ST_RSA_PRIVATE_KEY *pstPrivateKeyOut);
 ...
 *@li	\ref NDK_ERR "NDK_ERR" 			操作失败
*/
extern int (*NDK_AlgRSARecover)(uchar *psModule, int nModuleLen, uchar *psExp, uchar *psDataIn,
                                uchar *psDataOut);
...
*/
extern int
(*NDK_AlgRSAKeyPairVerify)(ST_RSA_PUBLIC_KEY *pstPublicKey, ST_RSA_PRIVATE_KEY *pstPrivateKey);
...
*/
extern int
(*NDK_SecGetMac)(uchar ucKeyIdx, uchar *psDataIn, int nDataInLen, uchar *psMacOut, uchar ucMod);
 ...
 *@li	\ref NDK_ERR_MACLLOC "NDK_ERR_MACLLOC" 	内存空间不足
*/
extern int (*NDK_SecCalcDes)(uchar ucKeyType, uchar ucKeyIdx, uchar *psDataIn, int nDataInLen,
                             uchar *psDataOut, uchar ucMode);
...
*/
extern int
(*NDK_SecVerifyPlainPin)(uchar ucIccSlot, uchar *pszExpPinLenIn, uchar *psIccRespOut, uchar ucMode,
                         uint unTimeoutMs);
...
*/
extern int
(*NDK_SecVerifyCipherPin)(uchar ucIccSlot, uchar *pszExpPinLenIn, ST_SEC_RSA_KEY *pstRsaPinKeyIn,
                          uchar *psIccRespOut, uchar ucMode, uint unTimeoutMs);
...
*/
extern int
(*NDK_SecLoadTIK)(uchar ucGroupIdx, uchar ucSrcKeyIdx, uchar ucKeyLen, uchar *psKeyValueIn,
                  uchar *psKsnIn, ST_SEC_KCV_INFO *pstKcvInfoIn);
...
*/
extern int
(*NDK_SecGetPinDukpt)(uchar ucGroupIdx, uchar *pszExpPinLenIn, uchar *psDataIn, uchar *psKsnOut,
                      uchar *psPinBlockOut, uchar ucMode, uint unTimeoutMs);
 ...
 *@li	\ref NDK_ERR "NDK_ERR"  	操作失败
*/
extern int (*NDK_SecGetMacDukpt)(uchar ucGroupIdx, uchar *psDataIn, int nDataInLen, uchar *psMacOut,
                                 uchar *psKsnOut, uchar ucMode);
...
*/
extern int
(*NDK_SecCalcDesDukpt)(uchar ucGroupIdx, uchar ucKeyVarType, uchar *psIV, ushort usDataInLen,
                       uchar *psDataIn, uchar *psDataOut, uchar *psKsnOut, uchar ucMode);
...
*/
extern int
NDK_SecRecover(uchar ucRsaKeyIndex, const uchar *psDataIn, int nDataLen, uchar *psDataOut);
...
//用于判断安全是否触发
extern int NDK_GetTamperStatus();
...
extern int
(*NDK_SecVerifyPIN)(uchar ucKeyID, uchar ucSKType, int nTSKLen, uchar *psTSK, uchar *psPan,
                    int PinBlockLen, uchar *psPinBlockIn, ST_SEC_RSA_KEY *pstRsaPinKeyIn,
                    uchar *psIccRespOut, uchar ucMode);
...
*/
extern int
(*NDK_PortRead)(EM_PORT_NUM emPort, uint unLen, char *pszOutBuf, int nTimeoutMs, int *pnReadLen);
...
*/
extern int
NDK_WlSendATCmd(const ST_ATCMD_PACK *pstATCmdPack, char *pszOutput, uint unMaxLen, uint unTimeout,
...
*/
extern int
NDK_TcpWrite(uint unFd, const void *pvInbuf, uint unLen, uint unTimeout, uint *punWriteLen);
...
*/
extern int
NDK_NetGetAddr(EM_COMM_TYPE emComtype, char *pszIp, char *pszMask, char *pszGateway, char *pszDns);
...
*/
extern int NDK_EthSetAddress(const char *pszIp, const char *pszMask, const char *pszGateway,
                             const char *pszDns);
...
*/
extern int
(*NDK_MagReadRaw)(uchar *pszTk1, ushort *pusTk1Len, uchar *pszTk2, ushort *pusTk2Len, uchar *pszTk3,
                  ushort *pusTk3Len);

extern int
(*NDK_MagReadRawData)(ENUM_MAG_DATA_TYPE type, ENUM_MAG_TRACK track, uint off, uint unLen,
                      uchar *tkdata, uint *pnReadlen);
...
*/
extern int
(*NDK_Iccrw)(EM_ICTYPE emIcType, int nSendLen, uchar *psSendBuf, int *pnRecvLen, uchar *psRecvBuf);
...
extern int (*NDK_RfidSetDetectType)(uint ucPiccType);
...
extern int (*NDK_RfidDetectWithCardType)(uint *psPiccType, int *pnDataLen, uchar *psDataBuf);
/**
 ...
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 		驱动调用错误(射频驱动接口RFID_IOCG_M1INTERAUTHEN调用失败返回)
*/
extern int (*NDK_M1ExternalAuthen)(int nUidLen, uchar *psUidBuf, uchar ucKeyType, uchar *psKeyData,
                                   uchar ucBlockNum);
...
/**
 *@brief	Felica卡 apdu超时时间设定
 *@param	timeout  超时时间（ms）（0-255）
 *@return
 *@li	\ref NDK_OK									操作成功
 *@li	\ref NDK_ERR_OPEN_DEV "NDK_ERR_OPEN_DEV" 	设备文件打开失败(射频设备文件打开失败)
 *@li	\ref NDK_ERR_IOCTL "NDK_ERR_IOCTL" 			驱动调用错误(射频驱动接口 IOCTL_RFID_SET_FELICA_TIMEOUT 调用失败返回)
 */
extern int (*NDK_FelicaSetTimeout)(uchar timeout);
...
/**
 *@brief    M0卡数据读写，传入和返回数据格式为Len+cmd+data
 *@param 	nSendlen 发送数据长度，psSendbuf 发送数据buf
 *@return   psRecebuf返回数据buf，pnRecvlen返回数据长度
 *@li   NDK_OK          操作成功
 *@li   NDK_ERR_PARA        参数错误(pnRfidtype为NULL)
...
*/
extern int
(*NDK_M0Authen_Release)(int nSendlen, uchar *psSendbuf, int *pnRecvlen, uchar *psRecebuf);
 ...
 */
extern int
(*NDK_RfidPiccApduInTransMode)(uchar *psSendbuf, int nSendlen, uchar *psRecebuf, int *pnRecvlen,
                               uint32_t timeout);
 ...
 */
typedef enum {
    RFID_RELEASE = 0x00, //production version mode
    RFID_AUTHENT = 0x01, //Authentication mode: PayPass\PayWave\QPBOC
    RFID_ID_CARD = 0x02, // ID Card mode
    RFID_MODE_END,
} EM_RF_MODE;
...
typedef enum {
    ...
    RFID_EMV_TRANSAC_A,
    RFID_EMV_TRANSAC_B,
    RFID_EMV_MODE_SELECT,
    RFID_EMV_CORRELATION_TEST,
    RFID_EMV_CARRIER_TEST,
    RFID_EMV_GET_PARAMER,
    RFID_EMV_SET_PARAMER,
    RFID_EMV_POLL,
    RFID_EMV_END,
}EMV_TEST_OPTION;
 ...
 *@li   \ref ..............
 */
extern int (*NDK_RfidConfig)(EM_RF_MODE rf_mode);

extern int (*NDK_RfidEMVTest)(EMV_TEST_OPTION option, int inSendlen, uchar *psSendbuf, int *pnRecvlen, uchar *psRecebuf);


extern int (*NDK_CEInit)(void);
extern int (*NDK_CEParamSet)(uint32_t param_type, uint32_t *param_len, uint8_t *param_buf);
extern int (*NDK_CEDataSync)(uint8_t file_type, uint8_t *data_buf, uint32_t *data_len);

typedef enum {
    CARD_EVENT_STATE = 0x01,        /*report card event : ndef has been read and so on*/
    CARD_WORK_STATE = 0x02,         /*report card work state to avoid suspend during card-reading  RFU*/
    CARD_EMU_ENSTATE = 0x03,        /*report card emulation enstate 0: diasble 1: enable*/
    CARD_TYPE_MAX,
}HCE_CARD_STATE_TYPE;
extern int (*NDK_CEGetState)(HCE_CARD_STATE_TYPE type, uint32_t *state);
...
extern int (*NDK_SysDevBacklightCtrl)(EM_BACKLIGHT_DEVICE mdevice, EM_DEVBACKLIGHT value);
/**
 *@brief 		设置是否允许自动进入休眠
...
*/
extern int NDK_AddDigitStr(const uchar *pszDigStr1, const uchar *pszDigStr2, uchar *pszResult,
                           int *pnResultLen);
...
*/
extern int
NDK_SubStr(const uchar *pszSouStr, int nStartPos, int nNum, uchar *pszObjStr, int *pnObjStrLen);
 ...
 *@li	\ref NDK_ERR "NDK_ERR"	操作失败（加载证书失败）
*/
extern int NDK_LoadClientPrivateKey(NDK_HANDLE pvHandle, const char *psFileName, int nFormat,
                                    char *pszPassword);
...
*/
extern int
NDK_SSLSend(NDK_HANDLE pvHandle, const char *psBuffer, uint unBufferLen, uint *punSendLen);
...
*/
extern int
NDK_SSLReceive(NDK_HANDLE pvHandle, void *pvBuffer, const uint unBufferLen, uint *punRecvLen);
...
extern int
NDK_RpcTransRW(unsigned char *DataIn, int LenIn, unsigned char *DataOut, int *LenOut, int maxlen,
               int utimeout);
...
extern int NDK_InitCom(int (*send)(unsigned char *data, int len, unsigned int respondTimeoutMs),
                       int (*recv)(unsigned char *buf, int buflen, unsigned int timeout),
                       char *info);
...
extern int (*NDK_SysTimeBeep_Ex)(uint unFrequency,uint unSeconds,uint unVolumn);
extern int (*NDK_SysSetBeepVol_Extern)(BEEP_TYPE type,uint unVolumn);
extern int (*NDK_SysGetBeepVol_Extern)(BEEP_TYPE type, uint *punVolumn);
/**
...
/**************************以下是SM相关**********************************************/
//整合libndkapism.so到libndkapi.so,原来libndkapi.so中没有声明的方法采用动态库打开方式调用,以下是相关函数.
/**
 *@brief 生成SM2密钥对
...
*/
extern int
(*NDK_AlgSM2Encrypt)(unsigned char *eccpubkey, unsigned char *Message, unsigned short MessageLen,
                     unsigned char *Crypto, unsigned short *CryptoLen);
...
*/
extern int
(*NDK_AlgSM2Decrypt)(unsigned char *eccprikey, unsigned char *Crypto, unsigned short CryptoLen,
                     unsigned char *Message, unsigned short *MessageLen);
...
*/
extern int
(*NDK_AlgSM2Verify)(unsigned char *pPublicKey, unsigned char *e, unsigned char *pSignedData);
...
*/
extern int
(*NDK_AlgSM2GenE)(unsigned short usID, unsigned char *pID, unsigned short usM, unsigned char *pM,
                  unsigned char *pubKey, unsigned char *pHashData);
 ...
 *@li	\ref NDK_ERR_PARA "NDK_ERR_PARA"	    参数非法
*/
extern int (*NDK_AlgSM4Compute)(unsigned char *pKey, unsigned char *pIVector, unsigned int len,
                                unsigned char *pSm4Input, unsigned char *pSm4Output,
                                unsigned char mode);
...
extern int (*NDK_SYS_RegisterEvent)(EM_SYS_EVENT eventNum, int timeOutMs,
                                    int (*notifyEvent )(EM_SYS_EVENT eventNum, int msgLen,
                                                        char *msg));
...
typedef struct {
    uint32_t  unFlickOn;//闪烁间隔时间，unFilckOn * 100ms
    uint32_t  unFlickOff;//0:亮完直接熄灭不闪烁；其它：闪烁
    uint32_t  unRverse[2];
}ST_NAPI_LED_FLICK;

//extern int (*NDK_LedSetFlickParam)(EM_LED emStatus, ST_NAPI_LED_FLICK FlickParam);

extern int (*NDK_LedLt1118Status)(EM_LED_LT1118 emStatus);


typedef enum{
    NDK_SEC_KEY_DES = 0,        /**<TDES Algorithm*/
    NDK_SEC_KEY_AES = 1         /**<AES Algorithm*/
}EM_NDK_SEC_KEY_ALG;
...
} EM_NDK_SEC_DUKPT_DERIVATE_USAGE;

typedef struct {
    EM_NDK_SEC_KEY_ALG KeyAlg; //指定派生密钥的算法
    EM_NDK_SEC_DUKPT_DERIVATE_USAGE DerivateUsage; //指定派生密钥
    int nKeyLen;//指定派生密钥的长度
}ST_NDK_SEC_DUKPT_DERIVATE_DATA;

typedef struct {
    uchar keyIndex;         //密钥ID
    uchar keyType;          //密钥类型
    uint32_t ksnlen;        //ksn长度
    uint8_t ksn[12];        //ksn
    uint32_t keylen;        //密钥实际长度
    uint32_t keydatalen;    //密钥数据长度
    uint8_t  keyvalue[32];  //密钥数据
} ST_AESDUKPT_KEYINFO;

typedef enum {
    NDK_SEC_CIPHER_MODE_ECB = 0,
    NDK_SEC_CIPHER_MODE_CBC,
    NDK_SEC_CIPHER_MODE_CFB,
    NDK_SEC_CIPHER_MODE_OFB
} EM_NDK_SEC_CIPHER_MODE;

typedef enum {
    NDK_CIPHER_ENCRYPT = 0,
    NDK_CIPHER_DECRYPT,
} EM_NDK_SEC_CIPHER_OPERATION;
...
typedef enum {
    ...
    NDK_SEC_MAC_TYPE_MAX
} EM_NDK_SEC_MAC_TYPE;

extern int (*NDK_SecCalcAesDukpt)(uchar ucGroupIdx, ST_NDK_SEC_DUKPT_DERIVATE_DATA * pstDerivateInfo, uchar *psIV, uint32_t usDataInLen, uchar *psDataIn, uchar *psDataOut,uchar *psKsnOut,EM_NDK_SEC_CIPHER_MODE ucCipherMode, EM_NDK_SEC_CIPHER_OPERATION ucCiperOperation);
extern int (*NDK_SecGetPinAesDukpt)(uchar ucGroupIdx, ST_NDK_SEC_DUKPT_DERIVATE_DATA * pstDerivateInfo, uchar *pszExpPinLenIn, uchar * psPan, uchar* psKsnOut, uchar *psPinBlockOut, uchar ucMode, uint unTimeoutMs);
extern int (*NDK_SecGetPinResultAesDukpt)(uchar *psPinBlock, uchar *psKsn, int *nStatus);
extern int (*NDK_SecGetMacAesDukpt)(uchar ucGroupIdx, ST_NDK_SEC_DUKPT_DERIVATE_DATA * pstDerivateInfo, uchar *psDataIn, int nDataInLen, uchar *psMacOut, uchar *psKsnOut, EM_NDK_SEC_MAC_TYPE ucMacType);
extern int (*NDK_SecGetAesDukptKsn)(uchar ucGroupIdx, uchar * psKsnOut);
extern int (*NDK_SecIncreaseAesDukptKsn)(uchar ucGroupIdx);
extern int (*NDK_SecLoadAesDukptKey)(uchar ucSrcKeyIdx, uchar ucSrcKeyType, ST_AESDUKPT_KEYINFO *keyinfo, ST_SEC_KCV_INFO * pstKcvInfoIn);
extern int (*NDK_SecLoadKeyRsa)(uchar ucSrcKeyIdx, uchar ucSrcKeyType, uchar ucRsaKeyIndex, ST_SEC_RSA_KEY *pstRsaKeyIn);
extern int (*NDK_SecRsaKeyRandomOut)(uchar ucSrcKeyIdx, uchar ucKeyType, uchar ucKeyIndex, uint32_t keylen, uchar *keyData, uint32_t *keyoutlen);
