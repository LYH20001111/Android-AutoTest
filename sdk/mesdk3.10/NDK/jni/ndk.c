...
//SM 整合libndkapism.so到libndkapi.so,原来libndkapi.so中没有声明的方法采用动态库打开方式调用,以下是相关函数.
...
int (*NDK_PrnModuleInit)(void);
int (*NDK_PrnCutterInit)(void);
...
int (*NDK_RfidSetDetectType)(uint ucPiccType);
int (*NDK_RfidPiccDetect)(uchar *psPiccType);
int (*NDK_RfidPiccActivate)(uchar *psPiccType, int *pnDataLen,  uchar *psDataBuf);
int (*NDK_RfidDetectWithCardType)(uint *psPiccType, int *pnDataLen,  uchar *psDataBuf);
...
int (*NDK_FelicaSetTimeout)(uchar timeout);
...
int (*NDK_M0Authen_Release)(int nSendlen, uchar *psSendbuf, int *pnRecvlen,  uchar *psRecebuf);
int (*NDK_RfidSetPiccParam)(uchar ucPiccparamtype, int pnParamlen, uchar *psParambuf);
int (*NDK_RfidPiccApduInTransMode)(uchar *psSendbuf, int nSendlen, uchar *psRecebuf, int *pnRecvlen, uint32_t timeout);
int (*NDK_RfidConfig)(EM_RF_MODE rf_mode);
int (*NDK_RfidEMVTest)(EMV_TEST_OPTION option, int inSendlen, uchar *psSendbuf, int *pnRecvlen, uchar *psRecebuf);
int (*NDK_CEInit)(void);
int (*NDK_CEParamSet)(uint32_t param_type, uint32_t *param_len, uint8_t *param_buf);
int (*NDK_CEDataSync)(uint8_t file_type, uint8_t *data_buf, uint32_t *data_len);
int (*NDK_CEGetState)(HCE_CARD_STATE_TYPE type, uint32_t *state);
//sec
...
int (*NDK_SysDevBacklightCtrl)(EM_BACKLIGHT_DEVICE mdevice, EM_DEVBACKLIGHT value);
...
int (*NDK_SysTimeBeep_Ex)(uint unFrequency,uint unSeconds,uint unVolumn);
int (*NDK_SysSetBeepVol_Extern)(BEEP_TYPE type,uint unVolumn);
int (*NDK_SysGetBeepVol_Extern)(BEEP_TYPE type,uint* unVolumn);
...
#define NAPI_ASYM
//int (*NAPI_SecGenerateKey)(EM_SEC_KEYIN_METHOD Method, ST_SEC_KEYIN_DATA *pstKGData, ST_SEC_KCV_DATA *pstKcvData);
int (*NAPI_SecAsymEncryption)(ST_SEC_ASYM_KEY_INFO *pstKeyinfo, EM_SEC_MD_TYPE MdAlg, EM_SEC_ASYM_ENCODING_MODE EncodingMode, EM_SEC_ASYM_CRYPTO_MODE CryptoMode, int nDataInLen, const unsigned char *psDataIn,  int* pnDataOutLen, unsigned char *psDataOut);
int (*NAPI_SecAsymDecryption)(ST_SEC_ASYM_KEY_INFO *pstKeyinfo, EM_SEC_MD_TYPE MdAlg, EM_SEC_ASYM_ENCODING_MODE EncodingMode, EM_SEC_ASYM_CRYPTO_MODE CryptoMode,int nDataInLen, const unsigned char *psDataIn,  int* pnDataOutLen, unsigned char *psDataOut);
int (*NAPI_SecAsymSign)(ST_SEC_ASYM_KEY_INFO *pstKeyinfo, EM_SEC_MD_TYPE MdAlg, EM_SEC_ASYM_ENCODING_MODE EncodingMode, int nHashLen, const unsigned char *psHash,  int* nSigLen, const unsigned char *psSig);
int (*NAPI_SecAsymVerify)(ST_SEC_ASYM_KEY_INFO *pstKeyinfo,  EM_SEC_MD_TYPE MdAlg, EM_SEC_ASYM_ENCODING_MODE EncodingMode, int nHashLen, const unsigned char *psHash,  int nSigLen, const unsigned char *psSig);
...
//Start AES DUKPT
int (*NDK_SecCalcAesDukpt)(uchar ucGroupIdx, ST_NDK_SEC_DUKPT_DERIVATE_DATA * pstDerivateInfo, uchar *psIV, uint32_t usDataInLen, uchar *psDataIn, uchar *psDataOut,uchar *psKsnOut,EM_NDK_SEC_CIPHER_MODE ucCipherMode, EM_NDK_SEC_CIPHER_OPERATION ucCiperOperation);
int (*NDK_SecGetPinAesDukpt)(uchar ucGroupIdx, ST_NDK_SEC_DUKPT_DERIVATE_DATA * pstDerivateInfo, uchar *pszExpPinLenIn, uchar * psPan, uchar* psKsnOut, uchar *psPinBlockOut, uchar ucMode, uint unTimeoutMs);
int (*NDK_SecGetPinResultAesDukpt)(uchar *psPinBlock, uchar *psKsn, int *nStatus);
int (*NDK_SecGetMacAesDukpt)(uchar ucGroupIdx, ST_NDK_SEC_DUKPT_DERIVATE_DATA * pstDerivateInfo, uchar *psDataIn, int nDataInLen, uchar *psMacOut, uchar *psKsnOut, EM_NDK_SEC_MAC_TYPE ucMacType);
int (*NDK_SecGetAesDukptKsn)(uchar ucGroupIdx, uchar * psKsnOut);
int (*NDK_SecIncreaseAesDukptKsn)(uchar ucGroupIdx);
int (*NDK_SecLoadAesDukptKey)(uchar ucSrcKeyIdx, uchar ucSrcKeyType, ST_AESDUKPT_KEYINFO *keyinfo, ST_SEC_KCV_INFO * pstKcvInfoIn);
int (*NDK_SecLoadKeyRsa)(uchar ucSrcKeyIdx, uchar ucSrcKeyType, uchar ucRsaKeyIndex, ST_SEC_RSA_KEY *pstRsaKeyIn);
int (*NDK_SecRsaKeyRandomOut)(uchar ucSrcKeyIdx, uchar ucKeyType, uchar ucKeyIndex, uint32_t keylen, uchar *keyData, uint32_t *keyoutlen);
//End AES DUKPT
#endif
...
#define DLSYM(lib, foo) {	\
	...
	if( lib == NULL || dlError || NULL == foo ){		\
		foo = NDK_Null;	\
		rc -= 1;	\
	  	 __android_log_print(ANDROID_LOG_INFO, TAG, "dlsym fail:  %s 0. "#foo"=%x ,ret will be %x\n", dlError,(int)foo,NDK_Null());	\
	} \	
	dlError = NULL;\
};\
...
/**
 *@brief   从系统动态库加载 POS-NDK 的 API函数指针
 *@return
 *@li   0 操作成功
 *@li   -1000 - libnlposapi.so 加载失败
 *@li   -2000 - libnl_ndk.so 加载失败
 *@li    -1~-1000 部分api加载失败，从0开始每失败一个返回值减1
*/
static int ndk_dlload(){
    int   rc;             /*  return codes            */
	functionLib = dlopen("libnlposapi.so",RTLD_LAZY); //在dlopen返回前，对于动态库中存在的未定义的变量(如外部变量extern，也可以是函数)不执行解析，就是不解析这个变量的地址。
	dlError = (char *)dlerror(); //dlerror可以返回出错信息，返回值为NULL时表示操作函数执行成功
	...
	DLSYM(functionLib,NDK_AlgSM2KeyPairGen);
	DLSYM(functionLib,NDK_AlgSM2Encrypt);
	DLSYM(functionLib,NDK_AlgSM2Decrypt);
	DLSYM(functionLib,NDK_AlgSM2Sign);
	DLSYM(functionLib,NDK_AlgSM2Verify);
	DLSYM(functionLib,NDK_AlgSM2GenE);
	DLSYM(functionLib,NDK_AlgSM3Start);
	DLSYM(functionLib,NDK_AlgSM3Update);
	DLSYM(functionLib,NDK_AlgSM3Final);
	DLSYM(functionLib,NDK_AlgSM3Compute);
	DLSYM(functionLib,NDK_AlgSM4Compute);
	...
	DLSYM(functionLib,NDK_M0Authen_Release);
	DLSYM(functionLib,NDK_RfidSetPiccParam);
	DLSYM(functionLib,NDK_RfidPiccApduInTransMode);
	DLSYM(functionLib,NDK_RfidConfig);
    DLSYM(functionLib,NDK_RfidEMVTest);
	DLSYM(functionLib,NDK_CEInit);
	DLSYM(functionLib,NDK_CEParamSet);
	DLSYM(functionLib,NDK_CEDataSync);
	DLSYM(functionLib,NDK_CEGetState);
	...
	DLSYM(functionLib,NDK_SecCalcAesDukpt);
	DLSYM(functionLib,NDK_SecGetPinAesDukpt);
	DLSYM(functionLib,NDK_SecGetPinResultAesDukpt);
	DLSYM(functionLib,NDK_SecGetMacAesDukpt);
	DLSYM(functionLib,NDK_SecGetAesDukptKsn);
	DLSYM(functionLib,NDK_SecIncreaseAesDukptKsn);
	DLSYM(functionLib,NDK_SecLoadAesDukptKey);
	DLSYM(functionLib,NDK_SecLoadKeyRsa);
	DLSYM(functionLib,NDK_SecRsaKeyRandomOut);
	...
	DLSYM(functionLib,NDK_SysDevBacklightCtrl);
    ...
    DLSYM(functionLib,NDK_SysGetBeepVol_Extern);
    ...
    char hardwareconfig[256];
    memset(hardwareconfig,0,sizeof(hardwareconfig));
    __system_property_get("ro.epay.hardwareconfig", hardwareconfig);
    NDK_PrnModuleInit = NDK_Null;
    __android_log_print(ANDROID_LOG_DEBUG, TAG, "hardwareconfig[%s]\n",hardwareconfig);
    if(strlen(hardwareconfig) >= 20){
        char printFlag[3];
        memset(printFlag,0,sizeof(printFlag));
        memcpy(printFlag,hardwareconfig+18,2);
        __android_log_print(ANDROID_LOG_DEBUG, TAG, "printFlag[%s]\n",printFlag);
        if(memcmp(printFlag,"FF",2) != 0){
            DLSYM(functionLib, NDK_PrnModuleInit);
			char gszDevModel[255];
			memset(gszDevModel,0,sizeof(gszDevModel));
			__system_property_get("ro.product.model", gszDevModel);
			__android_log_print(ANDROID_LOG_DEBUG, TAG, "gszDevModel1[%s]\n",gszDevModel);
			if(strncmp(gszDevModel,"CPOS",4) == 0){
                __android_log_print(ANDROID_LOG_DEBUG, TAG, "gszDevModel2[%s]\n",gszDevModel);
				DLSYM(functionLib, NDK_Script_Print);
			}
        }
    }
	DLSYM(functionLib, NDK_PrnCutterInit);
...
#include "__log.h"
#define LOG_TAG  "libndkapi"
#define LOGI(...) __android_log_print(ANDROID_LOG_DEBUG,LOG_TAG,__VA_ARGS__)
debug_api Udebug;
int Udebugopen2 = 0;//0211 第一位
int Udebuglevel2 = 0;
...
static void newlandmpos_printf(char * fmt,...)
{
	int len;
	va_list arg;
	char str[4096];
...
static void newlandmpos_printf_string(char *BUF,int LEN){
	int i;
	int len;
	int size;
	int temp;
	...
	for(i=0; i < LEN; ) {
		...
		size -= len;
		s[offset-1] = '\n';
		newlandmpos_printf("%s", s);
	}
}
static void newlandmpos_printf_string_null(char *BUF,int LEN){
	return;
}
static void newlandmpos_null(char * fmt,...){
	return;
}
static void DebugInit()
{
	...
	Udebuglevel2 = 2;
 	if(Udebuglevel2 > 0){
		Udebug.DEBUG_Levelone = newlandmpos_printf;
		Udebug.DEBUG_Leveltwo = newlandmpos_printf;
		Udebug.DEBUG_string_Levelone = newlandmpos_printf_string;
		Udebug.DEBUG_string_Leveltwo = newlandmpos_printf_string;
		Udebug.ERROR_MSG_LOG = newlandmpos_printf;
		Udebug.ERROR_MSG_LOG_String=newlandmpos_printf_string;
	}else{
		Udebug.DEBUG_Levelone = newlandmpos_null;
		Udebug.DEBUG_Leveltwo = newlandmpos_null;
		Udebug.DEBUG_string_Levelone = newlandmpos_printf_string_null;
		Udebug.DEBUG_string_Leveltwo = newlandmpos_printf_string_null;
		Udebug.ERROR_MSG_LOG = newlandmpos_null;
		Udebug.ERROR_MSG_LOG_String = newlandmpos_printf_string_null;
	}
}
int NDK_Init()
{
	static int flag = 0;
	if(flag == 1){
		__android_log_print(ANDROID_LOG_DEBUG, TAG, "NDK_Init Init flag_ok[%d]",flag);
		return 1;
	}
	flag = 1;
	DebugInit();
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    gJavaVM = vm;
    __android_log_print(ANDROID_LOG_DEBUG,TAG,"NDK_VERSION[%s]",NDK_VERSION);
